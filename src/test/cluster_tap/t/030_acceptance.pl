...
is($node->safe_psql('postgres',
	...
	'75', 'E1 pg_stat_cluster_wait_events returns 75 rows (spec-2.25 native-lock probe events included)');
...
is($node->safe_psql('postgres',
	...
	'75', 'E4 pg_stat_gcluster_wait_events returns 75 rows (single-node, spec-2.25 D12)');
