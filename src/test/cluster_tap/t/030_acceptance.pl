...
is($node->safe_psql('postgres',
	...
	'88', 'E1 pg_stat_cluster_wait_events returns 85 rows (spec-2.34 GCS block reliability events included)');
...
is($node->safe_psql('postgres',
	...
	'88', 'E4 pg_stat_gcluster_wait_events returns 85 rows (single-node, spec-2.34 D7)');
