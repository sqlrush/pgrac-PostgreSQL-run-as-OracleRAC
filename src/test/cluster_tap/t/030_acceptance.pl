...
is($node->safe_psql('postgres',
	...
	'71', 'E1 pg_stat_cluster_wait_events returns 70 rows (spec-2.20 GES S4 wait included)');
...
is($node->safe_psql('postgres',
	...
	'71', 'E4 pg_stat_gcluster_wait_events returns 70 rows (single-node, spec-2.20 included)');
