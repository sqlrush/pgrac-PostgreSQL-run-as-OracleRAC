...
# ----------
# L2: stage 1.3 baseline -- 2 rows (cluster_ctl + cluster_conf).
# ----------
is($node->safe_psql(
		'postgres',
		q{SELECT count(*) FROM pg_cluster_shmem}),
   '2',
   'L2 pg_cluster_shmem returns 2 baseline rows (cluster_ctl + cluster_conf)');

is($node->safe_psql(
		'postgres',
		q{SELECT string_agg(name, ',' ORDER BY name) FROM pg_cluster_shmem}),
   'pgrac cluster conf,pgrac cluster control',
   'L3 pg_cluster_shmem rows are exactly the two foundational regions');
...
is($node->safe_psql(
   ...
   '2',
   'L8 pg_cluster_state.shmem.region_count = 2');
...
is($node->safe_psql(
   ...
   '2',
   'L10 pg_cluster_state.shmem has 2 region.<name>.bytes keys (one per region)');
...
is($node->safe_psql(
   ...
   '2',
   'L11 pg_cluster_state.shmem has 2 region.<name>.owner keys (one per region)');
...
is($node->safe_psql(
   ...
   '24',
   'L15 total injection registry size is 24 (14 baseline + 8 sweep + 3 shared_fs/smgr is wrong, recompute below)');
...
# ----------
# L18: GUC max_regions=8 (boundary minimum) still admits 2 baseline regions.
# ----------
$node->stop;
...
is($node->safe_psql(
		...
		q{SELECT count(*) FROM pg_cluster_shmem}),
   '2',
   'L18 cluster.shmem_max_regions = 8 still admits the 2 baseline regions');
