...
#
#	  L1  fresh cluster startup:  pg_cluster_state.gcs has 31 keys
...
# L1 — pg_cluster_state.gcs surface has 31 keys.
is($node->safe_psql(
		'postgres',
		q{SELECT count(*) FROM pg_cluster_state WHERE category='gcs'}),
   '31',
   'L1 pg_cluster_state.gcs category has 31 keys (spec-2.34 D10)');
