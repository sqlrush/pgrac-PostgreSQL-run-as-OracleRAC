...
UT_TEST(test_cluster_wait_events_count_is_51)
{
	/*
	 ...
	 * If a future subsystem spec adds new cluster wait events, both
	 * the enum in wait_event.h and CLUSTER_WAIT_EVENTS_COUNT must move
	 * together, and this test number must be bumped in lockstep.
	 */
	UT_ASSERT_EQ(CLUSTER_WAIT_EVENTS_COUNT, 51);
}
...
{
	UT_PLAN(4);
	UT_RUN(test_cluster_wait_events_count_is_51);
